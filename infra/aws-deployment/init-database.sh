...
#
# Usage: bash infra/aws-deployment/init-database.sh
# ============================================
...
if [ ! -f "infra/database/init.sql" ]; then
    echo -e "${RED}❌ Run this script from the SysLab project root directory${NC}"
    exit 1
fi

# Load AWS config
if [ ! -f "infra/aws-deployment/aws-config.env" ]; then
    echo -e "${RED}❌ infra/aws-deployment/aws-config.env not found${NC}"
    echo -e "${YELLOW}Run: bash infra/aws-deployment/setup-infrastructure.sh first${NC}"
    exit 1
fi
...
if PGPASSWORD="$DB_PASSWORD" psql \
    ...
    -f infra/database/init.sql; then
...
if PGPASSWORD="$DB_PASSWORD" psql \
    ...
    -U postgres \
    -d system_design_simulator \
    -f infra/database/migrations/004_user_isolation_tables.sql; then

...
- [ ] Run: `bash aws-deployment/setup-infrastructure.sh`
- [ ] Wait 15-20 minutes for RDS and Redis to initialize
- [ ] Verify: `source aws-deployment/aws-config.env && echo $RDS_ENDPOINT`
...
- [ ] Build and push backend: `docker build -t [ECR_URI]/syslab-backend . -f backend/Dockerfile && docker push [ECR_URI]/syslab-backend`
- [ ] Build and push frontend: `docker build -t [ECR_URI]/syslab-frontend . -f frontend/Dockerfile && docker push [ECR_URI]/syslab-frontend`
- [ ] Verify images in ECR

### Phase 4: Database

- [ ] Register ECS task definitions
- [ ] Run: `bash aws-deployment/init-database.sh`
...
- [ ] Verify services are running: `bash aws-deployment/ecs-helper.sh services`
...
- [ ] Run health checks: `bash aws-deployment/ecs-helper.sh health [ALB_DNS]`
- [ ] Test API: `curl https://[ALB_DNS]/api/health`
- [ ] Test frontend: Open https://[ALB_DNS] in browser
- [ ] Test auth: Try signing up and logging in
- [ ] View logs: `bash aws-deployment/ecs-helper.sh logs backend`
...
# View configuration
cat aws-deployment/aws-config.env | grep -v "PASSWORD\|SECRET"
...
```bash
# View services
bash aws-deployment/ecs-helper.sh services
...
bash aws-deployment/ecs-helper.sh logs frontend

# Restart service
bash aws-deployment/ecs-helper.sh restart backend
bash aws-deployment/ecs-helper.sh restart frontend

# Scale service
bash aws-deployment/ecs-helper.sh scale backend 3
bash aws-deployment/ecs-helper.sh scale frontend 2

# Deploy new image
bash aws-deployment/ecs-helper.sh deploy backend $ECR_REGISTRY/syslab-backend:v1.0.0

# Health check
bash aws-deployment/ecs-helper.sh health [ALB_DNS]
```
...
aws logs get-log-events \
    --log-group-name /ecs/syslab-backend \
    --log-stream-name ecs/syslab-backend/xxxxx \
    --limit 50 \
    --region us-east-1
```
...
- `aws-deployment/AWS_DEPLOYMENT_GUIDE.md` - Complete guide with all steps
- `aws-deployment/GITHUB_ACTIONS_SETUP.md` - GitHub Actions configuration
- `aws-deployment/ecs-helper.sh help` - ECS helper command options
